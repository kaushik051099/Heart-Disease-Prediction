...
for i, col in enumerate(df_continuous.columns):
    ...
    graph = sns.histplot(data=df_continuous, x=col, bins=bin_edges, kde=True, ax=ax[x, y],
                         edgecolor='none', color='pink', alpha=0.6, line_kws={'lw': 3})
    ...
    ax[x, y].text(0.75, 0.9, textstr, transform=ax[x, y].transAxes, fontsize=12, verticalalignment='top',
                  color='white', bbox=dict(boxstyle='round', facecolor='#de7389', edgecolor='white', pad=0.5))
...
for i, col in enumerate(categorical_features):
    ...
    # Plot bar chart
    value_counts.plot(kind='barh', ax=ax[row, col_idx], width=0.7, color='#de7389')
...
for i,col in enumerate(categorical_features):
    ...
    # Define colormap
    cmp = ListedColormap(['#de7389', 'pink'])
...
for i, col in enumerate(continuous_features):
    sns.histplot(X_train[col], kde=True, ax=ax[0,i], color='red').set_title(f'Original {col}')
...
for i, col in enumerate(continuous_features):
    ...
    if X_train[col].min() > 0:
        ...
        sns.histplot(X_train[col], kde=True, ax=ax[1,i], color='#c24257').set_title(f'Transformed {col}')
    else:
        sns.histplot(X_train[col], kde=True, ax=ax[1,i], color='#eb2f3b').set_title(f'{col} (Not Transformed)')
